...
bool js_PluginReviewJS_PluginReview_userDidSignificantEvent(JSContext *cx, uint32_t argc, jsval *vp)
{
    ...
    JS_ReportError(cx, "js_PluginReviewJS_PluginReview_userDidSignificantEvent : wrong number of arguments");
    return false;
}
#elif defined(JS_VERSION)
...
bool js_PluginReviewJS_PluginReview_init(JSContext *cx, uint32_t argc, jsval *vp)
{
    ...
    JS_ReportError(cx, "js_PluginReviewJS_PluginReview_init : wrong number of arguments");
    return false;
}
#elif defined(JS_VERSION)
...
void js_PluginReviewJS_PluginReview_finalize(JSFreeOp *fop, JSObject *obj) {
    ...
    if (jsproxy) {
        ...
        sdkbox::PluginReview *nobj = static_cast<sdkbox::PluginReview *>(nproxy->ptr);
        if (nobj)
            delete nobj;
